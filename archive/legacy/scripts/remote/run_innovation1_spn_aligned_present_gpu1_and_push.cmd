...
set BRANCH=main
set RUN_ID=innovation1-spn-aligned-present-gpu1-20260607
set EXPECTED_ROWS=30
set RUN_ROOT=%ROOT%\%PROJECT_ID%-runs
set RUN_DIR=%RUN_ROOT%\%RUN_ID%
set ARCHIVE_WORK=%ROOT%\archive_work\spn_aligned_20260607
...
%PY% experiments\run_innovation_one_matrix.py ^
  --plan experiments\plans\innovation1_spn_aligned_present.csv ^
  --epochs 10 ^
  --batch-size 1024 ^
  --hidden-bits 64 ^
  --learning-rate 0.001 ^
...
copy "%RUN_DIR%\experiments\plans\innovation1_spn_aligned_present.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\plans\innovation1_spn_aligned_present.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo feature_comparison=raw_vs_spn_aligned_inverse_player>> results_archive\%RUN_ID%\run_manifest.txt
