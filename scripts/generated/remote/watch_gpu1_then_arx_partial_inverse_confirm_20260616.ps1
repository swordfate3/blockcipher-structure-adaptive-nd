...
$NextRun = "innovation1-arx-speck32-partial-inverse-r7-confirm-10seed-gpu1-20260616"
$NextScheduleScript = Join-Path $ProjectPath "scripts\generated\remote\schedule_innovation1_arx_speck32_partial_inverse_r7_confirm_10seed_gpu1_20260616.cmd"
$LogPath = Join-Path $LogDir "watch_gpu1_then_${NextRun}.log"
$PollSeconds = 600
$GpuIndex = 1
...
function Sync-Project {
    ...
    try {
        & git fetch origin refactor/model-project-structure 2>&1 | Tee-Object -FilePath $LogPath -Append
        & git checkout refactor/model-project-structure 2>&1 | Tee-Object -FilePath $LogPath -Append
...
function Test-ResultBranch {
    Push-Location $ProjectPath
    try {
        $branch = "results/$NextRun"
        $branchLine = & git ls-remote --heads origin $branch 2>&1
        $exitCode = $LASTEXITCODE
        if ($exitCode -eq 0 -and $branchLine -match [regex]::Escape($branch)) {
            Write-WatchLog "Result branch already exists for $NextRun"
            return $true
        }
        return $false
    }
...
while ($true) {
    Write-WatchLog "Checking GPU$GpuIndex before launching $NextRun"

    if (Test-ResultBranch) {
        exit 0
    }
    ...
    Write-WatchLog "GPU$GpuIndex appears free of matching training processes; syncing and launching $NextRun"
    Sync-Project
    Push-Location $ProjectPath
    try {
