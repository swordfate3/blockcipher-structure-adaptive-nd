...
set ARCHIVE_WORK=%ROOT%\archive_work\arx_speck32_carry_position_stats_r7r8_screen_gpu1_20260616
...
%PY% experiments\run_innovation_one_matrix.py --plan experiments\innovation1\plans\innovation1_arx_speck32_carry_position_stats_r7r8_screen.csv --epochs 16 --batch-size 512 --hidden-bits 32 --learning-rate 0.0001 --optimizer adamw --weight-decay 0.0001 --key-rotation-interval 1024 --sample-structure independent_pairs --integral-active-nibble 0 --device cuda:1 --dataset-cache-root dataset_cache --dataset-cache-chunk-size 4096 --checkpoint-metric val_auc --restore-best-checkpoint --early-stopping-patience 0 --early-stopping-min-delta 0 --progress-output logs\%RUN_ID%_progress.jsonl --output results\%RUN_ID%.jsonl > logs\%RUN_ID%_stdout.txt 2> logs\%RUN_ID%_stderr.txt
...
copy "%RUN_DIR%\experiments\innovation1\plans\innovation1_arx_speck32_carry_position_stats_r7r8_screen.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\innovation1\plans\innovation1_arx_speck32_carry_position_stats_r7r8_screen.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo validation=arx_speck32_carry_position_stats_r7r8_screen_gpu1>> results_archive\%RUN_ID%\run_manifest.txt
