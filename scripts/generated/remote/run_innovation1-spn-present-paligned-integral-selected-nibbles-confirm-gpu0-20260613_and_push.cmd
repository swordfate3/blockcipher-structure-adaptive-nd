...
set EXPECTED_ROWS=15
set RUN_ROOT=%ROOT%\%PROJECT_ID%-runs
set RUN_DIR=%RUN_ROOT%\%RUN_ID%
set ARCHIVE_WORK=%ROOT%\archive_work\spn_present_paligned_integral_selected_nibbles_confirm_gpu0_20260613
...
%PY% experiments\run_innovation_one_matrix.py ^
  --plan experiments\innovation1\plans\innovation1_spn_present_paligned_integral_selected_nibbles_confirm_gpu0.csv ^
  --epochs 12 ^
  --batch-size 512 ^
  --hidden-bits 64 ^
  --learning-rate 0.001 ^
  ...
  --sample-structure plaintext_integral_nibble ^
...
copy "%RUN_DIR%\experiments\innovation1\plans\innovation1_spn_present_paligned_integral_selected_nibbles_confirm_gpu0.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\innovation1\plans\innovation1_spn_present_paligned_integral_selected_nibbles_confirm_gpu0.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo sample_structure=plaintext_integral_nibble>> results_archive\%RUN_ID%\run_manifest.txt
...
echo validation=present_spn_paligned_integral_selected_nibbles_confirm_gpu0>> results_archive\%RUN_ID%\run_manifest.txt
