...
set RUN_ID=innovation1-spn-present-sinv-curriculum-r7-gpu0-20260616
set EXPECTED_ROWS=6
set RUN_ROOT=%ROOT%\%PROJECT_ID%-runs
set RUN_DIR=%RUN_ROOT%\%RUN_ID%
set ARCHIVE_WORK=%ROOT%\archive_work\spn_present_sinv_curriculum_r7_gpu0_20260616
...
%PY% experiments\run_innovation_one_matrix.py --plan experiments\innovation1\plans\innovation1_spn_present_sinv_curriculum_r7_screen.csv --epochs 16 --batch-size 128 --hidden-bits 32 --learning-rate 0.0001 --optimizer adam --weight-decay 1e-05 --key-rotation-interval 1024 --sample-structure zhang_wang_case2_mcnd --integral-active-nibble 0 --device cuda:0 --dataset-cache-root dataset_cache --dataset-cache-chunk-size 1024 --checkpoint-metric val_auc --restore-best-checkpoint --early-stopping-patience 0 --early-stopping-min-delta 0 --progress-output logs\%RUN_ID%_progress.jsonl --output results\%RUN_ID%.jsonl > logs\%RUN_ID%_stdout.txt 2> logs\%RUN_ID%_stderr.txt
...
copy "%RUN_DIR%\experiments\innovation1\plans\innovation1_spn_present_sinv_curriculum_r7_screen.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\innovation1\plans\innovation1_spn_present_sinv_curriculum_r7_screen.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo validation=present_sinv_curriculum_r7_gpu0>> results_archive\%RUN_ID%\run_manifest.txt
