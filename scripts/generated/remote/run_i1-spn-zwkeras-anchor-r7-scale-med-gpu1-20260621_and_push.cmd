...
set ARCHIVE_WORK=%ROOT%\archive_work\spn_zwkeras_anchor_r7_scale_med_gpu1_20260621
...
if exist "%PROJECT_DIR%\experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv" copy "%PROJECT_DIR%\experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv" "experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv"
...
%PY% experiments\run_innovation_one_matrix.py --plan experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv --epochs 50 --batch-size 1024 --hidden-bits 32 --learning-rate 0.0001 --optimizer adam --weight-decay 1e-05 --loss mse --lr-scheduler none --key-rotation-interval 0 --sample-structure zhang_wang_case2_official_mcnd --integral-active-nibble 0 --device cuda:1 --dataset-cache-root dataset_cache --dataset-cache-chunk-size 8192 --checkpoint-metric val_loss --restore-best-checkpoint --early-stopping-patience 8 --early-stopping-min-delta 0.0001 --progress-output %RUN_DIR%\logs\%RUN_ID%_progress.jsonl --output %RUN_DIR%\results\%RUN_ID%.jsonl > logs\%RUN_ID%_stdout.txt 2> logs\%RUN_ID%_stderr.txt
...
%PY% experiments\innovation1\validate_result_plan_alignment.py --plan experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv --results results\%RUN_ID%.jsonl --expected-rows %EXPECTED_ROWS% --output logs\%RUN_ID%_plan_alignment.json > logs\%RUN_ID%_plan_alignment_stdout.txt 2> logs\%RUN_ID%_plan_alignment_stderr.txt
...
copy "%RUN_DIR%\experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\innovation1\plans\innovation1_spn_present_zhang_wang2022_keras_official_anchor_scale_ladder_r7.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo epochs=50>> results_archive\%RUN_ID%\run_manifest.txt
...
echo early_stopping_patience=8>> results_archive\%RUN_ID%\run_manifest.txt
echo early_stopping_min_delta=0.0001>> results_archive\%RUN_ID%\run_manifest.txt
...
echo claim_scope=MEDIUM scale ladder only: test whether r7 AUC improves from 8k smoke to 64k/256k under Zhang/Wang 2022 official-anchor MCND; not formal or breakthrough evidence>> results_archive\%RUN_ID%\run_manifest.txt
echo launch_policy=short run_id avoids Windows path-length failures; run only this two-row ladder before any 1M formal expansion>> results_archive\%RUN_ID%\run_manifest.txt
echo validation=present_zhang_wang2022_keras_anchor_r7_scale_medium_gpu1>> results_archive\%RUN_ID%\run_manifest.txt
