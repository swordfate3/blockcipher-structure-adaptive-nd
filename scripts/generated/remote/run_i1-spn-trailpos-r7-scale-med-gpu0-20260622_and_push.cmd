...
set RUN_ID=i1-spn-trailpos-r7-scale-med-gpu0-20260622
set EXPECTED_ROWS=2
set RUN_ROOT=%ROOT%\%PROJECT_ID%-runs
set RUN_DIR=%RUN_ROOT%\%RUN_ID%
set ARCHIVE_WORK=%ROOT%\archive_work\spn_trailpos_r7_scale_med_gpu0_20260622
...
if exist "%PROJECT_DIR%\experiments\run_innovation_one_matrix.py" copy "%PROJECT_DIR%\experiments\run_innovation_one_matrix.py" "experiments\run_innovation_one_matrix.py"
...
if not exist src\blockcipher_ai_eval\experiments mkdir src\blockcipher_ai_eval\experiments
if exist "%PROJECT_DIR%\src\blockcipher_ai_eval\experiments\factories.py" copy "%PROJECT_DIR%\src\blockcipher_ai_eval\experiments\factories.py" "src\blockcipher_ai_eval\experiments\factories.py"
if not exist src\blockcipher_ai_eval\models mkdir src\blockcipher_ai_eval\models
if exist "%PROJECT_DIR%\src\blockcipher_ai_eval\models\registry.py" copy "%PROJECT_DIR%\src\blockcipher_ai_eval\models\registry.py" "src\blockcipher_ai_eval\models\registry.py"
if not exist src\blockcipher_ai_eval\models mkdir src\blockcipher_ai_eval\models
if exist "%PROJECT_DIR%\src\blockcipher_ai_eval\models\structure" xcopy "%PROJECT_DIR%\src\blockcipher_ai_eval\models\structure" "src\blockcipher_ai_eval\models\structure\" /E /I /Y
if not exist experiments\innovation1\plans mkdir experiments\innovation1\plans
if exist "%PROJECT_DIR%\experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv" copy "%PROJECT_DIR%\experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv" "experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv"
if not exist experiments\innovation1 mkdir experiments\innovation1
if exist "%PROJECT_DIR%\experiments\innovation1\validate_result_plan_alignment.py" copy "%PROJECT_DIR%\experiments\innovation1\validate_result_plan_alignment.py" "experiments\innovation1\validate_result_plan_alignment.py"
...
%PY% experiments\run_innovation_one_matrix.py --plan experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv --epochs 16 --batch-size 128 --hidden-bits 32 --learning-rate 0.0001 --optimizer adam --weight-decay 1e-05 --loss mse --lr-scheduler cyclic --max-learning-rate 0.002 --key-rotation-interval 1024 --sample-structure zhang_wang_case2_mcnd --integral-active-nibble 0 --device cuda:0 --dataset-cache-root dataset_cache --dataset-cache-chunk-size 8192 --checkpoint-metric val_auc --restore-best-checkpoint --early-stopping-patience 0 --early-stopping-min-delta 0 --progress-output %RUN_DIR%\logs\%RUN_ID%_progress.jsonl --output %RUN_DIR%\results\%RUN_ID%.jsonl > logs\%RUN_ID%_stdout.txt 2> logs\%RUN_ID%_stderr.txt
...
%PY% experiments\innovation1\validate_result_plan_alignment.py --plan experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv --results results\%RUN_ID%.jsonl --expected-rows %EXPECTED_ROWS% --output logs\%RUN_ID%_plan_alignment.json > logs\%RUN_ID%_plan_alignment_stdout.txt 2> logs\%RUN_ID%_plan_alignment_stderr.txt
...
copy "%RUN_DIR%\experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\innovation1\plans\innovation1_spn_present_trail_position_stats_r7_scale_med.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo lr_scheduler=cyclic>> results_archive\%RUN_ID%\run_manifest.txt
...
echo claim_scope=MEDIUM scale only: test whether the earlier r7 trail-position route that reached about 0.678 calibrated accuracy at 65536/class improves at 262144/class; not formal or breakthrough evidence>> results_archive\%RUN_ID%\run_manifest.txt
echo launch_policy=scale the exact trail-position structure-feature route, with global-stats control, before any 1M/class formal expansion>> results_archive\%RUN_ID%\run_manifest.txt
echo validation=spn_present_trail_position_stats_r7_scale_med_gpu0>> results_archive\%RUN_ID%\run_manifest.txt
