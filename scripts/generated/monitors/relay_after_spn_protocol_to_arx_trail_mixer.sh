...
UPSTREAM_RUN="innovation1-spn-present-protocol-spnaligned-scale-m-gpu1-20260616"
UPSTREAM_ROWS="12"
NEXT_RUN="innovation1-arx-speck32-trail-mixer-curriculum-r7r8-gpu1-20260615"
NEXT_ROWS="8"
...
SCHEDULE_SCRIPT="G:\lxy\blockcipher-structure-adaptive-nd\scripts\generated\remote\schedule_innovation1_arx_speck32_trail_mixer_curriculum_r7r8_gpu1_20260615.cmd"
...
while true; do
  ...
  if UV_CACHE_DIR=/tmp/uv-cache PYTHONDONTWRITEBYTECODE=1 uv run python scripts/monitor_remote_results.py --remote "${RESULT_REMOTE}" --once --run-id "${UPSTREAM_RUN}=${UPSTREAM_ROWS}"; then
    ...
    ssh -o BatchMode=yes -o ConnectTimeout=8 "${SSH_TARGET}" "cmd.exe /c cd /d G:\lxy\blockcipher-structure-adaptive-nd && ${SCHEDULE_SCRIPT}"
