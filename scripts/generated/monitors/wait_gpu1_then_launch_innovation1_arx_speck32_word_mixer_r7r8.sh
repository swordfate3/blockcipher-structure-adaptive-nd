...
SCHEDULE_SCRIPT="${SCHEDULE_SCRIPT:-G:\\lxy\\blockcipher-structure-adaptive-nd\\scripts\\generated\\remote\\schedule_innovation1_arx_speck32_word_mixer_r7r8_gpu1_20260615.cmd}"
...
remote_gpu1_busy() {
  ssh -o BatchMode=yes -o ConnectTimeout=8 "${SSH_TARGET}" \
    "powershell -NoProfile -Command \"\$procs = Get-CimInstance Win32_Process -Filter \\\"name='python.exe'\\\" | Where-Object { \$_.CommandLine -like '*run_innovation_one_matrix.py*' -and \$_.CommandLine -like '*--device cuda:1*' }; if (\$procs) { \$procs | Select-Object ProcessId,CommandLine | Format-List; exit 2 } else { Write-Host 'GPU1_TRAINING_IDLE'; exit 0 }\""
}
...
echo "waiting for remote GPU1 before launching ${RUN_ID}"
...
while true; do
  ...
  if remote_gpu1_busy; then
    echo "GPU1 appears idle for run_innovation_one_matrix.py; syncing and launching ${RUN_ID}"
    sync_remote_project
    launch_remote_run
    date '+%F %T' > "${LAUNCHED_FLAG}"
    echo "launched ${RUN_ID}; waiting for results branch"
  else
    echo "GPU1 still has an active matrix training process; sleeping"
  fi
