...
set BRANCH=main
set RUN_ID=innovation1-spn-input-ablation-present-gpu1-20260607
set EXPECTED_ROWS=24
set RUN_ROOT=%ROOT%\%PROJECT_ID%-runs
set RUN_DIR=%RUN_ROOT%\%RUN_ID%
set ARCHIVE_WORK=%ROOT%\archive_work\spn_input_ablation_20260607
...
%PY% experiments\run_innovation_one_matrix.py ^
  --plan experiments\plans\innovation1_spn_input_ablation_present.csv ^
  --epochs 10 ^
  --batch-size 1024 ^
  --hidden-bits 64 ^
  --learning-rate 0.001 ^
...
copy "%RUN_DIR%\experiments\plans\innovation1_spn_input_ablation_present.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\plans\innovation1_spn_input_ablation_present.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo validation=present_input_ablation_delta_inverse_p_and_full_pair_encodings>> results_archive\%RUN_ID%\run_manifest.txt
