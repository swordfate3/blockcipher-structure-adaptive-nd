...
set BRANCH=main
set RUN_ID=innovation1-spn-strict-crosskey-10seed-present-gpu0-20260608
set EXPECTED_ROWS=40
set RUN_ROOT=%ROOT%\%PROJECT_ID%-runs
set RUN_DIR=%RUN_ROOT%\%RUN_ID%
set ARCHIVE_WORK=%ROOT%\archive_work\spn_strict_crosskey_10seed_20260608
...
%PY% experiments\run_innovation_one_matrix.py ^
  --plan experiments\plans\innovation1_spn_strict_crosskey_10seed_present.csv ^
  --epochs 10 ^
  --batch-size 1024 ^
  --hidden-bits 64 ^
  --learning-rate 0.001 ^
...
copy "%RUN_DIR%\experiments\plans\innovation1_spn_strict_crosskey_10seed_present.csv" "results_archive\%RUN_ID%\"
...
echo plan=experiments\plans\innovation1_spn_strict_crosskey_10seed_present.csv>> results_archive\%RUN_ID%\run_manifest.txt
...
echo validation=strict_crosskey_encrypted_negative_10seed_present>> results_archive\%RUN_ID%\run_manifest.txt
